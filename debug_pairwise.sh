...
if [ $1 -lt $TOTAL ]; then
    ...
    ./mkth3.tcl $TESTFILES "$TH3CFG" > ../debugpr_$1/th3_generated_test.c
    cd ../debugpr_$1
    ...
    cp ../TypeChef-SQLiteIfdeftoif/optionstructs_ifdeftoif/pairwise/generated/Prod$configID.h id2i_include.h
    ...
    if cp $th3IfdeftoifDir/sqlite3_ifdeftoif_$TH3IFDEFNO.c sqlite3_ifdeftoif.c; then
        echo "pairwise debugging: jobid $1 ifdeftoif $TH3IFDEFNO; #ifConfig $IFCONFIGBASE on $TESTFILENO .test files in $TESTDIRBASE with th3Config $TH3CFGBASE at $(date +"%T")"
    else
